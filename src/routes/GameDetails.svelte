            ...
            <section class="participations-section">
                ...
                <div class="flex flex-col gap-6">
                    {#each participations as p (p.boxId)}
                        ...
                        {@const actualScoreForThisParticipation = (game.status === 'Resolution') ? resolve_participation_commitment(p, game.revealedS_Hex, game.seed) : null}
                            ...
                            {#if isMalformed && isCurrentUserParticipant}
                                <div class="info-block sm:col-span-2 lg:col-span-4 mt-4 mx-4 mb-4">
                                    <p class="text-xs {$mode === 'dark' ? 'text-gray-400' : 'text-gray-500'}">
                                        The funds will be awarded to the winner if the competition concludes successfully. If there is no winner, the funds will be allocated to the creator/resolver.
                                    </p>
                                </div>
                            {/if}
             ...
             <div class="modal-form-body">
                {#if currentActionType === 'submit_score'}
                    <div class="space-y-4">
                        ...
                        <p class="text-sm {$mode === 'dark' ? 'text-gray-400' : 'text-gray-600'} pt-2">A participation fee of <strong>{formatErg(game.participationFeeAmount)} ERG</strong> will be paid.</p>
